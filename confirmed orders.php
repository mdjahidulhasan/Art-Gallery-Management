...
<div class="abc">
	<center><h1>Confirmed Orders</h1></center>
	<hr class="a"><br><br><br>
	<ul>
<?php	
$sql = mysqli_query($link,"SELECT * FROM payment WHERE Confirmation=1");
while($row=mysqli_fetch_array($sql))
{
	...
	$art_id = $row["Arts_id"];
	?>
		<li> <b><?=$name?></b> (Username: <b><?=$user?></b> Email: <b><?=$email?></b> Contact No: <b><?=$num?></b> Address: <b><?=$add?></b>) has paid <?=$price?> to buy Art id <b><?=$art_id?></b>.<br> Bkash Number: <b><?=$bnum?></b> <br> Trx. Id: <b><?=$trx?></b><br>
		</li>
		<br>
		<?php 
        }
